...
<div class="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
    ...
    <div class="flex justify-between items-start mb-6">
        <div>
            ...
            <p class="text-gray-600">
                Kegiatan: <span class="font-semibold">{{ $activity->name }}</span><br>
                Program: <span class="font-semibold">{{ $group->program->name ?? '-' }}</span>
            </p>
        </div>
        <a href="{{ route('admin.sertifikasi.groups.show', ['sertifikasi' => $activity->id, 'group' => $group->id]) }}" class="text-gray-500 hover:text-gray-800">
    ...
    <div class="flex justify-between items-center mb-4">
        ...
        <div>
            <a href="{{ route('admin.kuesioner.results', $group->id) }}" class="bg-green-500 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-green-600">Lihat Hasil Jawaban</a>
        ...
        @forelse ($questions as $question)
            <div draggable="true" data-id="{{ $question->id }}" class="bg-gray-50 p-4 rounded-md cursor-move border">
                ...
                <div class="text-right mt-2">
                    <form action="{{ route('admin.kuesioner.destroy', $question) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus pertanyaan ini?');" class="inline-block">
                        @csrf
                        @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    saveBtn.addEventListener('click', () => {
        ...
        fetch('{{ route("admin.kuesioner.updateOrder") }}', {
            method: 'POST',
            headers: {
