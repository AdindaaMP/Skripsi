...
<div class="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
    ...
    <div class="flex justify-between items-start mb-8">
        <div class="flex items-center gap-6">
            ...
            <div>
                ...
                <div class="flex gap-2 mt-4">
                    <a href="{{ route('admin.trainer.edit', $trainer->id) }}" class="bg-yellow-500 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-yellow-600">Ubah</a>
                    <form action="{{ route('admin.trainer.destroy', $trainer->id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus trainer ini? Role akan dikembalikan menjadi user biasa.')">
                        @csrf
                        @method('DELETE')
        ...
        <a href="{{ route('admin.trainer.index') }}" class="text-gray-400 hover:text-gray-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            @empty
                <div class="text-center py-8 md:col-span-3">
                    ...
                    <p class="text-gray-500">Trainer ini belum mengajar di program manapun.</p>
                </div>
            @endforelse
        </div>
    </div>
    
    {{-- Saran dari Kuesioner --}}
     <!-- Saran dari Peserta -->
    <div class="mt-8">
        <h2 class="text-xl font-semibold text-gray-700 mb-4">Saran dari Peserta</h2>
        <div class="space-y-4">
            @forelse ($suggestionsFromPeserta as $program => $saranList)
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-bold text-gray-800 mb-2">{{ $program }}</h3>
                    <ul class="list-disc list-inside space-y-2 text-sm text-gray-700">
                        @foreach (collect($saranList)->take(5) as $saran)
                           <li class="italic">"{{ $saran }}"</li>
                        @endforeach
                    </ul>
                    @if(count($saranList) > 5)
                        <div class="mt-3">
                            <button onclick="this.nextElementSibling.classList.toggle('hidden'); this.textContent = this.textContent.includes('Tampilkan') ? 'Sembunyikan' : 'Tampilkan Semua Saran...';" class="text-blue-500 text-sm font-semibold hover:underline">Tampilkan Semua Saran...</button>
                            <ul class="list-disc list-inside space-y-2 text-sm text-gray-700 mt-2 hidden">
                                @foreach (collect($saranList)->skip(5) as $saran)
                                   <li class="italic">"{{ $saran }}"</li>
                                @endforeach
                            </ul>
                        </div>
    ...
    <div class="mt-8">
        ...
        <div class="space-y-4">
            @forelse ($suggestionsFromProctor as $program => $saranList)
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-bold text-gray-800 mb-2">{{ $program }}</h3>
                    <ul class="list-disc list-inside space-y-2 text-sm text-gray-700">
                        @foreach (collect($saranList)->take(5) as $saran)
