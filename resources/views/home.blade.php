...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-1">Anda login sebagai: <span class="font-semibold">{{ ucfirst(Auth::user()->role) }}</span></p>
        </div>
    </div>

    {{-- MENU BARU KHUSUS UNTUK PROCTOR --}}
    @if(Auth::user()->role === 'proctor')
    <div class="mb-6">
        <a href="{{ route('proctor.ketercapaian') }}" class="inline-block bg-white p-4 rounded-lg shadow-sm border hover:shadow-md transition-shadow">
        ...
        @forelse ($groups as $group)
            <div class="bg-white p-5 rounded-lg shadow-sm flex items-center justify-between">
                ...
                <div class="flex items-center gap-4">
                    <img src="{{ asset($group->activity->logo) }}" alt="Logo Kegiatan" class="h-12 w-12 object-contain rounded-md bg-gray-100 p-1">
                    <div>
                        <p class="font-bold text-gray-800">{{ $group->activity->name }}</p>
                        <p class="text-sm text-gray-600">
                            <span class="font-semibold">Kelompok:</span> {{ $group->name }} |
                            <span class="font-semibold">Trainer:</span> {{ optional($group->trainers->first())->name ?? 'N/A' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2">
                    @if(Auth::user()->role === 'trainer')
                        ...
                        <a href="{{ route('trainer.results', $group->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg text-sm">Lihat Hasil & Saran</a>
                        ...
                        @if($availableQuestions->count() > 0)
                            ...
                            @else
                                <a href="{{ route('groups.evaluasi.form', $group->id) }}" 
                    ...
                    @if(Auth::user()->role === 'proctor')
                        ...
                        <a href="{{ route('proctor.monitoring', $group->id) }}" class="bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg text-sm">Halaman Pengawasan</a>
                    ...
                    @if(Auth::user()->role === 'user')
                        ...
                        @else
                            <a href="{{ route('groups.evaluasi.form', $group->id) }}" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-lg text-sm">
                                Isi Evaluasi
                            </a>
                        @endif
                    @endif
